...
def _create_consumer(queue, futures, loop=None):
    ...
    while True:
        ...
        future = futures if (step is None) else futures[step]       # error (index out of range) step = 3
...
class MultiTaskSampler(Sampler):
    ...
    def __init__(
        self, env_name, env_kwargs, batch_size, policy, baseline, env=None, seed=None, num_workers=1
    ):
        super().__init__(env_name, env_kwargs, batch_size, policy, seed=seed, env=env)
    ...
    def sample_tasks(self, num_tasks):
        return self.env.unwrapped.sample_tasks(num_tasks)   # type: ignore
    ...
    def sample_async(self, tasks, **kwargs):
        ...
        num_steps = kwargs.get("num-steps", 1)
